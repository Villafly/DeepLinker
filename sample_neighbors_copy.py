...
def get_neighbors(idx,sample_size,labels,adj, batch_size= 256):
    ...
    for ids in np.array_split(node_list,10):
        multi_results.append(pool.apply_async(sample_neighbor,args=(ids, adj, sample_size[0])))
    pool.close()
    pool.join()  
    multi_results = [item.get() for item in multi_results]
    node_neighbor_matrix = reduce(lambda x,y: np.vstack((x,y)), multi_results)
    node_nei_list = node_neighbor_matrix.flatten().tolist()
    time2 = time.time()
    print("Nei sampling costs {}s".format(time2-time1))
 
    pool2 = Pool()
    multi_results = []
    for ids in np.array_split(node_nei_list,10):
        multi_results.append(pool2.apply_async(sample_neighbor,args=(ids, adj, sample_size[0])))
    pool2.close()
    pool2.join()   
    multi_results = [item.get() for item in multi_results]
    node_neighbor_2_matrix = reduce(lambda x,y: np.vstack((x,y)), multi_results)
    #time2 = time.time()
    #print("Nei sampling costs {}s".format(time2-time1))
    return_list = []
    for chunk_id, ids in enumerate(_array_split(idx, batch_size)):
        ...
        node_list_batch,node_neighbor_matrix_batch,node_neighbor_2_matrix_batch = __batch_to_torch(np.array(node_list_batch),node_neighbor_matrix_batch, node_neighbor_2_matrix_batch)
        batch = [node_list_batch, node_neighbor_matrix_batch, node_neighbor_2_matrix_batch, targets]
